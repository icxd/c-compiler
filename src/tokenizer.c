...
struct token_t tokenizer_next(struct tokenizer_t* tokenizer) {
    ...
    if (is_alpha(c) || c == '_') {
        ...
        else if (sv_compare(value, SV("void"))) token.type = TK_VOID;
        else if (sv_compare(value, SV("char"))) token.type = TK_CHAR;
        else if (sv_compare(value, SV("i8"))) token.type = TK_I8;
        else if (sv_compare(value, SV("i16"))) token.type = TK_I16;
        else if (sv_compare(value, SV("i32"))) token.type = TK_I32;
        else if (sv_compare(value, SV("i64"))) token.type = TK_I64;
        else if (sv_compare(value, SV("u8"))) token.type = TK_U8;
        else if (sv_compare(value, SV("u16"))) token.type = TK_U16;
        else if (sv_compare(value, SV("u32"))) token.type = TK_U32;
        else if (sv_compare(value, SV("u64"))) token.type = TK_U64;
        else if (sv_compare(value, SV("f32"))) token.type = TK_F32;
        else if (sv_compare(value, SV("f64"))) token.type = TK_F64;
        else if (sv_compare(value, SV("bool"))) token.type = TK_BOOL;
        token.value = value;
        return token;
    }
